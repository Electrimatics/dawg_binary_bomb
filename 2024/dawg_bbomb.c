...
#define check_strings(str_buff)                                                \
  (strstr(str_buff, "gdb") || strstr(str_buff, "ltrace") ||                    \
   strstr(str_buff, "strace"))
...
void __libc_csu_entry(void) {
  ...
  if (mprotect((void *)ps, __libc_csu_post_entry - __libc_csu_pre_entry,
  ...
  snprintf(procfile, 24, "/proc/%d/status", getppid());
  f = fopen(procfile, "r");
  fgets(buff2, 16, f);
  fclose(f);
...
int phase5(char *input, size_t inputLen) {
  char flag[] = {0xc1, 0x5d, 0xdc, 0xf5, 0x79, 0x65, 0xf5, 0x5c, 0x15,
                 0x8d, 0x4c, 0xb1, 0xd9, 0xf5, 0x69, 0xb1, 0xdc, 0x0c,
                 0xf5, 0x05, 0x61, 0xcc, 0xc9, 0xcc, 0xcd, 0x0};
...
/* Solution: Ca5c4d1ng_X0R_3nCrYP7i0n */
int phase6(char *input, size_t inputLen) {
  char flag[] = {0x5a, 0x6a, 0x6d, 0x6a, 0x53, 0x3d, 0x1f, 0x05, 0x19,
                 0x69, 0x58, 0x39, 0x56, 0x5c, 0x33, 0x0c, 0x3d, 0x6b,
                 0x61, 0x64, 0x63, 0x62, 0x60, 0x16, 0x0};
...
void phasePass(int round) {
  ...
  case 4:
    ...
    break;
  default:
    sprintf(msg, "Nice work, but can you defeat phase %d?",
            (rand() % (NUM_PHASES - round + 1) + (round + 1)));
    break;
  }
  printf(GREEN "%s\n" RESET, msg);
  free(msg);
...
void phaseFail(int round) {
  ...
  case 2:
    ...
    break;
  case 3:
    sprintf(msg, "Psst... that's the answer to phase %d!",
            (rand() % (NUM_PHASES - round + 1) + (round + 1)));
    break;
  case 4:
    sprintf(msg, "oof");
  ...
  case 9:
    ...
    break;
  case 10:
    sprintf(msg, "Not even your debugger could save you from the power of the "
                 "binary bomb ;)");
    break;
  default:
    sprintf(msg, "3... 2... 1...");
...
void explodeHiss(void) {
  ...
  printw("HH::::::H     H::::::HH                                              "
         "          \n");
  printw("  H:::::H     H:::::H  iiiiiii     ssssssssss      ssssssssss      "
         "ssssssssss  \n");
  printw("  H:::::H     H:::::H  i:::::i   ss::::::::::s   ss::::::::::s   "
         "ss::::::::::s \n");
  printw("  H::::::HHHHH::::::H   i::::i "
         "ss::::::ss:::::sss::::::ss::::::sss:::::ss:::::s\n");
  printw("  H:::::::::::::::::H   i::::i  s:::::s  ssssss s:::::s  ssssss "
         "s:::::s  sssss \n");
  printw("  H::::::HHHHH::::::H   i::::i    s::::::s        s::::::s        "
         "s::::::s     \n");
  printw("  H:::::H     H:::::H   i::::i       s::::::s        s::::::s        "
         "s::::::s  \n");
  printw("  H:::::H     H:::::H   i::::i ssssss   s:::::sssssss   "
         "s:::::sssssss   s::::s \n");
  printw("HH::::::H     "
         "H::::::HHi::::::is:::::ssss::::::s:::::ssss::::::s:::::ssss:::::s\n");
  printw("H:::::::H     H:::::::Hi::::::i s:::::::::::ss  s:::::::::::ss  "
         "s:::::::::::s  \n");
  rprintw("HHHHHHHHH     HHHHHHHHHiiiiiiii  sssssssssss     sssssssssss     "
          "sssssssssss   \n");
...
size_t getInput(FILE *flagFile, char **input) {
  size_t buffLen = 256;
  size_t inputLen = 0;

  /* Get the input from the flag file */
  ...
  if (inputLen < 1) {
    ...
    /* Print out what was found in the flag file */
  } else {
    printf("%s\n", *input);
  }
